...
def cumsum_trick(x, geom_feats, ranks):
    x = x.cumsum(0)  # 累加分布
    kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)
    kept[:-1] = (ranks[1:] != ranks[:-1])  # 每个voxel只保留一个特征？？？？

    x, geom_feats = x[kept], geom_feats[kept]
    x = torch.cat((x[:1], x[1:] - x[:-1]))  # 减去x.cumsum(0)的累计和

    return x, geom_feats

def bev_pool(feats, coords, B, D, W, H):
    assert feats.shape[0] == coords.shape[0]

    ranks = (  # 每个points对应唯一的voxel的rank
        coords[:, 0] * (H * D * B)
        + coords[:, 1] * (D * B)
        + coords[:, 2] * B
        + coords[:, 3]
    )
    indices = ranks.argsort()  # 排序
    feats, coords, ranks = feats[indices], coords[indices], ranks[indices]  # 与顺序对齐

    x = QuickCumsumCuda.apply(feats, coords, ranks, B, D, W, H)  # bev pool
