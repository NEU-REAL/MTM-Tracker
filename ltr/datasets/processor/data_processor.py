...
class DataProcessor(object):
    def __init__(self, processor_configs, sr_point_cloud_range, tp_point_cloud_range, training, num_point_features):
        self.sr_point_cloud_range = sr_point_cloud_range
        self.tp_point_cloud_range = tp_point_cloud_range

        self.training = training
        self.num_point_features = num_point_features
        self.mode = 'train' if training else 'test'
        self.sr_grid_size = self.tp_grid_size = self.voxel_size = None
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        if data_dict.get('gt_boxes', None) is not None and config.REMOVE_OUTSIDE_BOXES and self.training:
            mask = box_utils.mask_boxes_outside_range_numpy(
                data_dict['gt_boxes'], self.sr_point_cloud_range, min_num_corners=config.get('min_num_corners', 1)
            )
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        if data_dict is None:
            sr_grid_size = (self.sr_point_cloud_range[3:6] - self.sr_point_cloud_range[0:3]) / np.array(config.VOXEL_SIZE)
            self.sr_grid_size = np.round(sr_grid_size).astype(np.int64)

            tp_grid_size = (self.tp_point_cloud_range[3:6] - self.tp_point_cloud_range[0:3]) / np.array(config.VOXEL_SIZE)
            self.tp_grid_size = np.round(tp_grid_size).astype(np.int64)

            self.voxel_size = config.VOXEL_SIZE
            return partial(self.transform_points_to_voxels, config=config)

        if self.voxel_generator is None:
            self.sr_voxel_generator = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,
                coors_range_xyz=self.sr_point_cloud_range,
                num_point_features=self.num_point_features,  # x\y\z\intensity
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL,
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode],
            )
            self.tp_voxel_generator = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,
                coors_range_xyz=self.tp_point_cloud_range,
                num_point_features=self.num_point_features,  # x\y\z\intensity
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL,
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode],
            )
        ...
        search_voxel_output = self.sr_voxel_generator.generate(search_points)
        if isinstance(search_voxel_output, dict):
            ...
            # search_voxels = [voxel_num, max_points_num, point_channel]
            # search_coordinates = [voxel_num, 3]
            # search_num_points = voxel_num
        ...
        template_voxel_output = self.tp_voxel_generator.generate(template_points)
